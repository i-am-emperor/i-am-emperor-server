...
export class BaseDao<TClass, TInterface extends Document> {
  ...
  async find(
    ...
    const document = await this.model.findOne(conditions, undefined, option);
  ...
  async bulkCreate(docs: DocumentDefinition<TInterface>[]): Promise<TClass[]> {
    const documents = await this.model.create(docs);
